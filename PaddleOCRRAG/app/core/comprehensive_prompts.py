...
你必须返回标准JSON格式,不要包含任何其他文字说明。"""

    
    CERTIFICATE_ANALYSIS_PROMPT = Template("""请分析以下证书信息,并返回JSON格式的加分结果。
...
请严格按照以下JSON格式返回结果,不要包含任何其他文字:
{
    ...
    BATCH_CALCULATION_PROMPT = Template("""请根据以下学生信息计算综测成绩,返回标准JSON格式。
...
请严格按照以下JSON格式返回结果,不要包含任何其他文字:
{
    ...
    RULE_RETRIEVAL_PROMPT = Template("""请从知识库中检索与以下问题相关的综测规则,并返回JSON格式结果。
...
请严格按照以下JSON格式返回结果:
{
    ...
    WEIGHT_CONFIG_PROMPT = Template("""请根据知识库中的配置,返回综测成绩计算的权重配置。
...
请严格按照以下JSON格式返回结果:
{
    ...
    EXCEL_FILL_PROMPT = Template("""请根据以下学生数据,生成用于填充Excel表格的JSON数据。
...
请严格按照以下JSON格式返回,确保数据可以按学号匹配填充到Excel:
{
    ...
    def get_certificate_analysis_prompt(cls, certificate_text: str, student_info: Dict = None) -> str:
        """获取证书分析提示词"""
        return cls.CERTIFICATE_ANALYSIS_PROMPT.substitute(
    ...
    def get_batch_calculation_prompt(cls, student_id: str, student_name: str, 
        ...
        """获取批量计算提示词"""
        return cls.BATCH_CALCULATION_PROMPT.substitute(
    ...
    def get_rule_retrieval_prompt(cls, query: str) -> str:
        """获取规则检索提示词"""
        return cls.RULE_RETRIEVAL_PROMPT.substitute(query=query)
    
    @classmethod
    def get_weight_config_prompt(cls, academic_year: str, semester: str) -> str:
        """获取权重配置提示词"""
        return cls.WEIGHT_CONFIG_PROMPT.substitute(
    ...
    def get_excel_fill_prompt(cls, raw_data: str, student_list: List) -> str:
        """获取Excel填充提示词"""
        return cls.EXCEL_FILL_PROMPT.substitute(
            raw_data=raw_data,
